  ...
  try {
    ...
    const projects = await Promise.all(
      projectsSnapshot.docs.map(async (doc) => {
        ...
        const [codesSnapshot, attendeesSnapshot, redemptionsSnapshot] =
          await Promise.all([
            getDocs(
              query(
                collection(db, "codes"),
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "A project with this slug already exists. Please choose a different name or slug.",
        },
        { status: 400 }
      );
    }
    ...
    const projectData = {
      ...
      eventDate: validatedData.eventDate
        ? Timestamp.fromDate(validatedData.eventDate)
        : null,
      createdAt: now,
      updatedAt: now,
    };
